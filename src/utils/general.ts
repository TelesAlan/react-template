...
export function orderBy(arr: any[], is_asc: boolean, key?: string) {
    return arr.sort((a, b) => {
        return ((key ? a[key] : a) < (key ? b[key] : b) ? -1 : 1) * (is_asc ? 1 : -1)
    });
}
export function orderByDate(arr: any[], is_asc: boolean, key?: string) {
    return arr.sort((a, b) => {
        return ((key ? new Date(a[key]).getTime() : new Date(a).getTime()) < (key ? new Date(b[key]).getTime() : new Date(b).getTime()) ? -1 : 1) * (is_asc ? 1 : -1)
    });
}
export function generateCascadeOptions(arr: any, childrens: string[]) {
    return Object.keys(arr).map(key => {
        return {
            "label": key,
            "value": key,
            "children": childrens.map(item => ({ "label": item, "value": item }))
        }
    });
}
export function groupByDevicesInflux(arr: any[], key2distinct: string) {
    let temp: any = {};

    arr.forEach(item => {
        if (!temp.hasOwnProperty(item[key2distinct])) {
            temp[item[key2distinct]] = [];
        }

        temp[item[key2distinct]].push(item);
    });
    return temp;
}
export function distinct_json(json: any[], key: string | number): any[] {
    return json.map(v => v[key]).filter((value, index, self) => self.indexOf(value) === index)
}
export function generateAlias(texto: string): string {
    function removeAccent(texto: string): string {
    ...
    return removeAccent(texto).replace(/ /g, "_").toLowerCase();
}
export function existInJSON(json: any[], key: string | number, value: any) {
    return json.find(item => item[key] === value) ? true : false;
}
